...
public class CriminalIntentJSONSerializer {
	...
	public void saveCrimes(ArrayList<Crime> crimes) throws JSONException, IOException {
		//Build an array in JSON and add all the crimes to the array
	...
	public ArrayList<Crime> loadCrimes() throws JSONException, IOException {
		...
		try {
			...
			//Now build the crime array using the json array
			for (int i = 0; i < jsonarray.length(); i++) {
				crimes.add(new Crime(jsonarray.getJSONObject(i)));
			}
		} catch (FileNotFoundException e) {
			//Ignore this exception, it occurs when you start the app fresh
		} finally {
